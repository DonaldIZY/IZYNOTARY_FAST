<template>
          ...
          <v-row>
            <v-col cols="12">
              <required-document
                ...
                v-model:file="cniOfRightsHolders"
              ></required-document>
            </v-col>
            <v-col cols="12">
              <required-document
                label="CNI du donateur"
                v-model:file="cniOfTheDonor"
              ></required-document>
            </v-col>
            <v-col cols="12">
              <required-document
                label="Extrait d'acte de naissance des ayants droits"
                v-model:file="birthCertificateOfTheRightsHolders"
              ></required-document>
            </v-col>
          </v-row>
          <h4 class="mt-5 mb-3">
  ...
  <div class="ma-4 d-flex justify-end">
    ...
    <confirmation-modal
      text="Voulez-vous enregistrer la procédure de succession de biens immobiliers ?"
      :open="openConf"
...
const handleProcedure = async () => {
  ...
  procedureData.append(
    "folderNum",
    procedureNumGenerator("Succession de biens immobiliers", lastId)
  );
  ...
  const requiredFiles = {
    ...
    birthCertificateOfTheRightsHolders:
      birthCertificateOfTheRightsHolders.value,
  };
  ...
  try {
    const date = await $fetch(
      API_SERVER_URL + `/folders/transferOfRealEstate`,
      {
        method: "POST",
        body: procedureData,
      }
